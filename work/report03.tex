...
\begin{itemize}
    ...
    \item \textbf{Mathematical Formulation Stability:} This refers to the geometric precision of the generative trajectories\cite{92}. We quantify stability using the Mean Squared Error (MSE), which measures the ``Path Deviation'' of the model's predicted trajectory from a theoretical straight line\cite{90}.
\end{itemize}
By analyzing these metrics, we seek to demonstrate how path ``straightening'' directly impacts hardware efficiency and model stability\cite{90}.

\section{Theoretical Framework: Stochastic vs. Deterministic Transport}
The architectural divergence between these frameworks is rooted in the choice between Stochastic Differential Equations (SDE) and Ordinary Differential Equations (ODE)\cite{92}. DDPM models the generative process as a reverse-time SDE, incorporating ``stochastic jitter'' to remain stable\cite{88}. Conversely, Flow Matching and Rectified Flow utilize an ODE framework to define a smooth, deterministic velocity field\cite{91}.
...
A fundamental challenge in high-dimensional transport is ``Path Entanglement.'' Even with a linear interpolation between $X_0$ and $X_1$, different pairs of noise and data often result in crossing trajectories\cite{91}. Since a neural network $v_\theta(x, t)$ is a single-valued function, it is mathematically forced to learn the average (mean) of all target directions at any intersection point\cite{91}. This forcing leads to a time-variant, curved velocity field, which manifests empirically as ``Blur'' in generated samples and high inference costs due to the need for many small integration steps\cite{91}.

\subsection{The Mathematical Link: Eulerian vs. Lagrangian}
Flow Matching and Rectified Flow describe the same physical transport but through different lenses\cite{92}. Flow Matching typically adopts an Eulerian (Field-centric) perspective, focusing on the continuity equation\cite{92}:
\begin{equation}
\frac{\partial \rho_t(x)}{\partial t} + \nabla \cdot (\rho_t(x)v(x, t)) = 0
\end{equation}
to preserve probability mass\cite{92}. Rectified Flow adopts a Lagrangian (Particle-centric) perspective, focusing on the trajectories of individual random variable paths $X_t$\cite{92}. The goal of Rectified Flow is to solve the entanglement problem by explicitly straightening these paths\cite{90}.

\section{The Rectified Flow Framework: From Chaos to Straight Highways}
Rectified Flow provides a structured, three-phase approach to eliminate path curvature, enabling extreme inference efficiency\cite{90}.

\begin{enumerate}
    \item \textbf{Phase 1: Initial Training/Rectification (1-RF):} The model learns a velocity field $v_\theta$ to minimize the least-squares error along linear paths\cite{90}. While this establishes a valid flow, the paths still cross and curve because $X_0$ and $X_1$ are coupled independently\cite{90}.
    \item \textbf{Phase 2: The Reflow Step (2-RF):} This is the critical ``Flow-Rewiring'' phase\cite{90}. By simulating the ODE from Phase 1, we find the deterministic endpoints $X^{\text{rect}}_1$ for each $X_0$\cite{90}. This process re-pairs the noise and data to minimize the quadratic transport cost\cite{90}:
    \begin{equation}
    \min_\pi \int \|x_0 - x_1\|^2 d\pi(x_0, x_1)
    \end{equation}
    The result is a set of non-intersecting, perfectly straight trajectories\cite{90}.
    \item \textbf{Phase 3: Jump:} Once the paths are straightened and the velocity field becomes time-independent ($v(x, t) \approx v(x, 0)$), a student model $\alpha_\phi(X_0)$ is trained to predict the exact ODE endpoint $X_1$ in a single step, effectively skipping the simulation path entirely\cite{90}.
\end{enumerate}

\noindent\textbf{Analysis:} The ``Reflow'' step is what allows the generative process to collapse from many steps to one\cite{90}. By minimizing the quadratic transport cost, the model regularizes the velocity field toward homogeneity\cite{90}. This removes the necessity of an iterative solver to compensate for direction changes during inference\cite{90}.

\section{Implementation Architecture and Evaluation Metrics}
To ensure a rigorous ``fair-comparison'' environment, we utilized a routing node architecture where all models originate from a shared initial noise distribution $\mathcal{N}(0, \mathbf{I})$\cite{87}.

\subsection{Evaluation Architecture}
\begin{itemize}
    \item \textbf{Routing Node:} Distributes the shared latent noise to three execution paths: the Stochastic Reverse Chain (DDPM), the Deterministic ODE Solver (FM), and the 1-Step Jump Network (RF)\cite{87}.
    \item \textbf{Backbone Integration:} We employ a convolutional U-Net conditioned on the time variable $t$\cite{119}. The scalar $t$ is first transformed via sinusoidal position encoding\cite{119}.
    \item \textbf{MLP Specifics:} The embedding is processed by a Multi-Layer Perceptron using a specific \texttt{nn.Sequential(nn.Linear(...), nn.ReLU())} architecture\cite{121}. The output is projected and added to the U-Net feature maps, allowing the network to dynamically adjust to the current timestep\cite{119}.
\end{itemize}

\subsection{Measurement Layer}
We evaluate the performance across three specific metrics\cite{87}:
\begin{itemize}
    \item \textbf{NFE $\downarrow$:} The total number of network executions per image\cite{87}.
    \item \textbf{Latency (ms) $\downarrow$:} Physical wall-clock time on hardware\cite{87}.
    \item \textbf{MSE (Path Deviation) $\downarrow$:} The mean squared error measuring drift from a linear trajectory\cite{87}.
\end{itemize}
...
During the execution of this course project, several AI assistants and typesetting platforms were utilized to support development, writing, and documentation:
\begin{enumerate}
    \item For editing the report and for structuring, we used Antigravity.
    \item For typesetting, we used Overleaf.
    \item For describing the compute environment in Appendix A, we used Antigravity.
    \item For code development, we also used ChatGPT and Antigravity.
    \item Antigravity and NotebookLM were used to generate text in all chapters.
\end{enumerate}

\begin{thebibliography}{99}
\bibitem{87} 
Instructor: Ivan Dokmani\'{c}, Term: Spring 2026.
...
\newblock In {\em Medical Image Computing and Computer-Assisted Intervention -- MICCAI 2015}, pages 234--241, 2015.
...
\caption{Visual sample and trajectory comparisons for the Dual Moon dataset. The subplots display: (1) Target Moons Data (True Data Distribution); (2) DDPM Samples (Stochastic Reverse Sampling); (3) Flow Matching in 50 Steps (ODE Euler Integration); (4) Flow Matching in 1 Step (Overshoots Curved Paths); (5) Rectified Flow in 1 Step (Follows Straight Paths Perfectly); and (6) Trajectories Comparison showing curved Flow Matching paths (green, dashed) against straightened Rectified Flow paths (red, solid).}
...
\section{Comparing outputs of the to Celeb064 evaluation}
